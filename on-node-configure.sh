...
MOGUL_ACTIVATION_ID=$3
CRYOSPARC_INSTALL_PATH=${4:-/shared/cryosparc}
CUDA_INSTALL_PATH=${5:-/shared/cuda}
CUDA_VERSION=${6:-11.3.1}
CUDA_LONG_VERSION=${7:-11.3.1_465.19.01}
CRYOSPARC_VERSION=${8:-latest}

# Install the AWS CLI
pip3 install --upgrade awscli boto3

# Activate Mogul
echo "Checking if we need to activate mogul.."
if [[ -d $CSDS_DIR ]]; then
  echo "Activating mogul..."
  $CSDS_DIR/CSD_2022/bin/ccdc_activator -a -k $MOGUL_ACTIVATION_ID
fi
